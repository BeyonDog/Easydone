...
// Fix wrong </motion.div> in GoGmt if still div/div
s = s.replace(
  /(<div className="modal-back" onMouseDown=\{\(\) => setGoGmtModal\(null\}\">[\s\S]*?<\/div>\n        )<\/motion\.div>/,
  "$1</motion.div>",
);
// Fix snapshot save primary button mislabeled 取消
s = s.replace(
...
console.log("pass2 applied", n, "pairs; remaining ???", rem);
if (rem > 0) {
  s.split("\n").forEach((line, i) => {
